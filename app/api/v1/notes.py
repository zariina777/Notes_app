...
@router.get("/pagination", response_model=List[NoteInDB])
async def get_notes_endpoint(
        limit: int = Query(10, le=30),
        offset: int = Query(0),
        db: AsyncSession = Depends(get_db)
):
    notes = await get_notes_service(db, limit=limit, offset=offset)
    return notes

@router.put("/(note_1d}/archive", response_nodel=NoteInDB)
...
@router.put("/(note_id)/active", response_model=NoteInDB)
async def active_note_endpoint(note_id: int, db: AsyncSession - Depends(get_db)):
    db_note = await active_note_service(db, note_id)
    if not db_note:
        raise HTTPException(status_code=404, detail="Note not found")
    return db_note