# описываем работу со всеми нашими заметками(создание редакция удаление)
...
async def get_notes_pagination(db: AsyncSession, limit: int, offset: int):
    query = select(Note).where(Note.is_deleted == False).limit(limit).offset(offset).order_by(Note.id)
    result = await db.execute(query)
    return result.scalars().all()


async def active_note_in_db(db: AsyncSession, db_note: Note):
    db_note.status = "active"
    await db.comit()
    return db_note
